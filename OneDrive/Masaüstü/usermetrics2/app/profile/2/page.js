...
export default function Home() {
    ...
    useEffect(() => {
        console.log(profile.companytype)
    },[])
                ...
                <div className={'flex w-[350px] sm:w-[442px] mt-[30px] items-center  flex-col gap-[10px]'}>
                    <div className={' flex flex-col gap-[8px] justify-start w-full h-fit'}>
                        <h1 className={'font-normal flex flex-row items-center gap-[5px] text-[16px]'}>
                            Company Name <span className={`${profile.companyname === "" ? "flex" : "hidden" } text-[#E45270]`}>*</span>
                        </h1>
                    ...
                    <div className={' flex flex-col gap-[8px] justify-start w-full h-fit'}>
                        <h1 className={'font-normal text-[16px] flex flex-row items-center gap-[5px] '}>
                            Company Type <span className={`${profile.companytype === "" ? "flex" : "hidden" } text-[#E45270]`}>*</span>
                        </h1>
                        <CreatableSelect onChange={(e) => {setProfile({
                            ...profile,
                            companytype: e.value
                        })}}  placeholder={'Select Company Type'} className={' !h-[48px] cursor-pointer'} options={companytype}  />

                    </div>

                    <div className={' flex flex-col mt-[23px] gap-[8px] justify-start w-full h-fit'}>
                        <h1 className={'font-normal text-[16px] flex flex-row items-center gap-[5px] '}>
                           Team Size <span className={`${profile.teamsize === "" ? "flex" : "hidden" } text-[#E45270]`}>*</span>
                        </h1>
                        <CreatableSelect onChange={(e) => {setProfile({
                            ...profile,
                            teamsize: e.value
                        })}}  placeholder={'Select Team Size'} className={' !h-[48px] cursor-pointer'} options={teamsize}  />
